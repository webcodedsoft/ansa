...
import { MORE_CAMPAIGN_TEMPLATES } from "./campaign-templates.more";
import {
  confirmOrMove,
  EVERY_DAY,
  HOME_HOURS,
...
export type { CampaignTemplate } from "./campaign-templates.shape";
 ...
 */
const FOUNDING: readonly CampaignTemplate[] = [
  // ------------------------------------------------------------------ Property
  {
...
/**
 * The whole catalogue: the founding seventeen and the wider set, in one list.
 *
 * Two files rather than one because a single file of sixty-seven campaigns is not something
 * anybody can navigate, and the split is by when they were written rather than by anything
 * a reader would look for — so the gallery treats them as one and sorts by sector.
 */
export const CAMPAIGN_TEMPLATES: readonly CampaignTemplate[] = [...FOUNDING, ...MORE_CAMPAIGN_TEMPLATES];
