...
Tenants one through ten are onboarded by hand (PRD §11, and §1.2 makes a self-serve builder
...
`docs/TENANT_CONFIGURATION.md` says what the fields mean and which of them a tenant is not
...
no check anywhere that it happened: a tenant provisioned without it looks completely correct
in the database and never receives a call.

**2. Create the tenant row.** Owner role, because `dialled_number` is the ingress routing
table and not the tenant's to write.

```
MIGRATION_DIRECT_URL=... node tools/tenant/provision.mjs "<organisation name>" <+E164>
```

It prints the tenant id. Everything after this is keyed on it.

**3. Write the configuration file.** One JSON object, the whole configuration rather than a
patch. The shape and the bounds are in `docs/TENANT_CONFIGURATION.md`; the header comment of
`tools/tenant/config.mjs` carries a worked example of each block. Keep it out of the
repository — `tools/tenant/local/` is gitignored for this.

**4. Seal any credentials it refers to.** The config names a credential by reference; the
value never appears in it.

```
TENANT_ID=... node tools/tenant/config.mjs credential <ref> bearer <token>
TENANT_ID=... node tools/tenant/config.mjs credential <ref> signing <shared-secret>
```
...
```
TENANT_ID=... node tools/tenant/config.mjs publish tools/tenant/local/<name>.json "<why>"
```
...
the tenant row and deliberately not reachable from the onboarding tool (§5 of the
...
the owner, `config.mjs` runs as `ansa_app` inside the tenant's own scope, and step 6 is raw
...
a tenant wrongly is to do all seven steps and forget step 1, and the symptom is a phone that
...
log nobody is watching. The second tenant's first published version was in exactly that state
for several minutes.
...
a patch history is unreadable — but it means the file in `tools/tenant/local/` is the only
copy of the truth, and losing it means reconstructing the configuration from
`config.mjs show <version>`.

**`publish_tenant_config` grows an argument per migration and old callers rot silently.**
...
**Keyterms are the field most likely to be filled in wrongly by a well-meaning tenant.** The
