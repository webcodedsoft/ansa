...
That is the right shape, and it is a genuine differentiator. The incumbents hand a organization
...
In a single-organization system that is a quality argument. In a multi-tenant one it is a safety
boundary, because **a organization's instructions must never be able to switch off a guarantee.**

Consider a organization who writes *"skip the readback, our customers find it slow."* If readback
...
| Enforced in code — a organization cannot override | Shaped by organization config |
|---|---|
...
| `organization_id` isolation via RLS (R7.2) | Which tools are registered |
...
correction is promoted into the eval corpus **and** into a per-organization keyterm **and** into a
normalizer test case.

That loop compounds, and it compounds *across organizations*: one organization's mishearing makes the
...
Not one string in a file. Five layers, composed per call, so per-organization becomes swapping
one of them.
...
| `organization` | organization | per config version | "You answer for Kano General Insurance. Warm, not chatty." |
...
**Storage.** Files are a stepping stone; Postgres is the destination. R7.5 requires organization
...
**The organization layer is bounded on purpose.** It layers on top of the base; it never replaces
it. A organization supplies persona and rules, not the whole prompt. That is what keeps the left
column of §1 true.

---

## 4. Organization configuration — a typed shape, not free text

```
organization_config (versioned; every call records which version served it)
...
Validated at registration. That validation is what stops a organization disabling something by
...
What "skills" means in practice — reusable capabilities, enabled per organization — is what the
...
1. **Per-organization keyterms.** Small, immediate. The list is hardcoded in `media.gateway.ts`;
...
4. **Prompt layering (§3).** Useful, but the least load-bearing of these five: the base is
   currently strong enough and no second organization exists.
...
marketplace, no analytics beyond raw call inspection. First ten organizations are onboarded by
hand (§11, Phase 1).
...
organization plumbing — it is one company's calls working brilliantly.**
