...
//
// `provision.mjs` creates the tenant and claims its number. This puts a person in it, and
...
//
//   MIGRATION_DIRECT_URL=... node tools/tenant/owner.mjs <tenant-uuid> <email> [role]
//
...
const [tenantId, email, role = "owner"] = process.argv.slice(2);
if (!tenantId || !email) {
  throw new Error("usage: owner.mjs <tenant-uuid> <email> [owner|admin|member]");
}
...
const { rows: tenants } = await client.query("select id, name from tenants where id = $1", [tenantId]);
if (tenants.length === 0) {
  await client.end();
  throw new Error(`no tenant ${tenantId} — run provision.mjs first`);
}
...
await client.query(
  `insert into invitations (tenant_id, email, role, token_hash, expires_at)
   values ($1, $2, $3, $4, $5)`,
  [tenantId, email.toLowerCase(), role, minted.hash, expiresAt],
);

await client.end();

console.log(`invited ${email} to ${tenants[0].name} as ${role}, expiring ${expiresAt.toISOString()}\n`);
