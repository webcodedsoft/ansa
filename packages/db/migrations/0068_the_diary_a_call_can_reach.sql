-- The calendar an agent books into, carried down to the call.
--
-- 0067 added `agents.appointment_calendar_id`. This is the other half: the four config
-- readers have to return it, or the column is a setting nothing can read — the shape of
-- mistake that has already cost this repo three features (see the notes in 0064).
--
-- Two questions worth answering before reading the SQL.
--
-- Why a column and not published configuration? Rule 4 protects the *words a caller hears*
-- from being drafted and unpublished. A calendar id is not words; it is a link, like
-- `organization_number_routing` or `escalation_to_number`, both of which live here for the
-- same reason. Pointing an agent at a different diary does not change a syllable it says,
-- and routing a booking through a draft is not a failure mode that exists — there is no
-- draft calendar.
--
-- Why append rather than insert in a sensible place? `agent_config_for_agent` and
-- `agent_config_for_organization` both delegate with `select * from
-- app.agent_config_for_id(...)`, which is positional. A column added in the middle would
-- shift every field after it and typecheck clean the whole way down.
--
-- The ACLs at the bottom are not optional bookkeeping: `drop function` plus `create
-- function` grants EXECUTE to PUBLIC afresh, and two of these take an id straight from a
-- request path. 0056 learned this the expensive way and 0057 wrote it down.
