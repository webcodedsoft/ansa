-- How this organisation's own names are said.
--
-- The voice mangles what it has never heard: a company's name, its branches, its products,
-- the surnames of the people who answer when it transfers. The platform carries a built-in
-- Nigerian lexicon (`@ansa/normalizer`, `NIGERIAN_LEXICON`) — states, towns, districts,
-- the common names of every region, the household institutions — and this column holds
-- what one organisation adds to it: `[{"term": "Oakhaven", "sayAs": "oak-hay-ven",
-- "ipa": "…"}]`. An entry here wins over a built-in one for the same term.
--
-- Read by the call-configuration functions, so it rides to the call with everything else in
-- one round trip, and applied one step before the voice. The language model never sees it
-- and must not: it chooses which words to say, this decides how they sound.
--
-- The four functions are dropped and recreated with the column appended, exactly as 0082
-- did for closed dates, and the ACLs are restored at the end for the same reason.

alter table organizations
  add column if not exists pronunciations jsonb not null default '[]'::jsonb;

do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'organizations_pronunciations_is_array') then
    alter table organizations add constraint organizations_pronunciations_is_array check (
      jsonb_typeof(pronunciations) = 'array'
    );
  end if;
end $$;

comment on column organizations.pronunciations is
  'This organisation''s additions to the built-in pronunciation lexicon: [{term, sayAs, ipa?}]. Applied to what the voice says, never read by the model.';
...
create function app.agent_config_for_id(agent uuid)
  returns table(
    ...
    flow jsonb, authoring_mode text, appointment_calendar_id uuid, record_calls boolean, pronunciations jsonb)
  language sql
  stable
  security definer
...
as $function$
  select t.id, a.id, a.name, a.keyterms, a.voice_id, a.greeting, a.persona, a.instructions,
         ...
         t.record_calls,
         /* The organisation's own pronunciation list, merged over the built-in Nigerian one
            before any sentence reaches the voice. Never read by the language model. */
         t.pronunciations
    from agents a
