...
- [x] **Per-organisation rate limits (R7.4, 2026-08-22).** `rateLimit` gained a
      `by: "organization"` mode and a second guard, `OrganizationRateLimitGuard`, registered
      after `ApiGuard`.

      Two guards rather than one branch, because the ordering carries the meaning. The
      anti-abuse limiter runs *before* authentication so a guessing attack cannot make the
      process spend a hundred milliseconds of scrypt — which is exactly why it cannot know who
      is calling. A quota is a different question, about one signed-in organisation calling a
      vendor on a loop, and it cannot be answered before the principal exists.

      Keyed on the organisation *and* the route. On the organisation alone, a busy tool sandbox
      would exhaust the budget for reading a call log — coupling that only surfaces during an
      incident.

      Applied where the code had already named the gap: the tool sandbox, the connector sample
      and the test call all carried a comment reading "keyed by address rather than by
      organisation, which is all `rateLimit` can express". Three colleagues holding a button
      down is one load on one endpoint, and three addresses behind one office NAT was never the
      unit anybody meant.

      Not billing, and the guard says so: in-memory, per process, so two instances allow twice
      the rate and a window boundary allows a short burst. A quota anybody is charged against
      belongs in the database, counted per call.

      Seven tests. Six on the guard and one that reads `api.module.ts` to pin the registration
      order, because that is the failure nothing else can see — put the quota before `ApiGuard`
      and there is no principal, so every organisation limit silently stops applying.
      Mutation-verified by swapping the two lines.
